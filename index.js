...
router.get('/checkPremium/:id', async (req, res) => {
  try {
    const post = await paymentTable
      .find({ userId: req.params.id, paymentstatus: 'done' })
      .sort({ _id: -1 })
      .limit(1);
...
router.post('/paydetails', async (req, res) => {
  try {
    ...
    if (post[0].paymentid === req.body.pyid) {
      const ress = await paymentTable.findByIdAndUpdate(
        post[0]._id,
        {
          paymentstatus: 'done',
        },
...
router.post('/buynow', async (req, res) => {
  try {
    ...
    const REDIRECT_URL = 'https://small-talk-aj.netlify.app/successful';
...
router.post('/profile-register', async (req, res) => {
  try {
    ...
    if (!exist) {
      ...
      const exis = await profileTable.findOne({
...
router.post('/updateph', async (req, res) => {
  try {
    const ress = await userDetailsTable.findByIdAndUpdate(
      req.body.userid,
      {
...
router.get('/updateById/:id', async (req, res) => {
  try {
    ...
    const ress = await conversationTable.findByIdAndUpdate(
      post._id,
      {
        status: !post.status,
      },
...
router.post('/create-pdf', async (req, res) => {
  try {
    ...
    const conversation = await conversationTable.findById(
    ...
    if (!exist) {
      ...
      res
        ...
        .send({
          ...
          name: usedet.name,
          userID: usedet._id,
          userEmail: usedet.email,
          imageUrl: usedet.imageUrl,
          phone: usedet.phone,
        });
    } else {
      const id = exist._id;
